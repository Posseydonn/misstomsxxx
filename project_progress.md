...
- Для запросов расписания в `server/routes/chat.js` используется отдельная детерминированная ветка (`handleAvailabilityRequest`) вместо свободной генерации ответа.
- Добавлен разбор запроса по врачу и дате (`parseAvailabilityQuery`): поддержка форматов даты `dd.mm`, `dd/mm`, `dd-mm`, а также `сегодня`/`завтра`.
- В `onlineDoctors` остаются только врачи, подтвержденные каталогом клиники (`DOCTOR_CATALOG`), исключен fallback формата `Врач #ID`.
- Для сценария "врач есть в каталоге, но без online-id" возвращается:
  - `onlineDoctors = []`
  - выбранный врач в `offlineDoctors`
- Для фильтра `врач + дата` снят лимит превью: возвращается полный набор слотов за день (`maxSlotsPerDoctor: Number.MAX_SAFE_INTEGER`).
- Исправлена нормализация русских токенов в парсере (`normalizeRu`): корректная обработка `ё/е` и диапазона кириллицы.

### Frontend: чат и логика выбора слота
- Цепочка автодействия слота внедрена полностью:
  - `ChatWidget -> ChatMessage -> AvailabilityCard`
  - при клике по времени автоматически отправляется сообщение: `Запиши меня к {врач} на {дата} в {время}`.
- При `isTyping=true` кнопки времени блокируются (без повторных/дублирующих отправок).
- Добавлен сценарий CTA для отсутствия онлайн-окон: передача заявки администратору через одно сообщение.

### UX/UI карточки свободных окон (новая версия)
- Карточка в единой стилистике сайта: `rounded-2xl`, светлый surface, `shadow-card`, узкая контрастная шапка `bg-clinic-gradient`.
- Фильтр врачей внутри карточки:
  - горизонтальный скролл-чипов,
  - мини-аватар,
  - активный врач в `clinic-red`, неактивные — нейтральные.
- Панель выбранного врача: имя, роль, счетчик `всего`.
- Фильтр дат внутри карточки:
  - чипы `dd.mm + день недели`,
  - активная дата в `clinic-teal`.
- Время сгруппировано по периодам дня:
  - `Утро (09:00–11:59)`
  - `День (12:00–15:59)`
  - `Вечер (16:00–19:59)`
- Внутри периодов — плотная grid-кладка time chips для большей информативности на экран.
- Блок врачей без онлайн-окон через `details/summary`, свернут по умолчанию.
- Пустые состояния реализованы явно:
  - нет onlineDoctors,
  - нет окон на выбранную дату,
  - невалидные даты (скрытие date chips + fallback текст).
- Защита от "пустых кружков":
  - не рендерятся чипы с пустым/мусорным label,
  - нормализация имен/ролей/дат перед выводом.

### Фото врачей
- Создан единый маппинг `doctorName -> photo` в `src/components/chat/doctorPhotos.ts`.
- Если фото не найдено, используется круглый fallback с инициалами.
...
### Ключевые измененные файлы (этап Фазы 13)
